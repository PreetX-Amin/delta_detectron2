...
class Backbone(dc.DCModule, metaclass=ABCMeta):
    """
...
class BottleneckBlock(dc.DCModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.norm1 = BatchNorm2d(bottleneck_channels)
        self.activation1 = dc.DCActivation('relu')
        ...
        self.norm2 = BatchNorm2d(bottleneck_channels)
        ...
        self.norm3 = BatchNorm2d(out_channels)
        self.add = dc.sparse_layers.DCAdd('relu')
    ...
    def half(self):
        self.norm1.half()
        super(BottleneckBlock, self).half()
...
class DeltaResNet(dc.DCModule):
    """
    Implement :paper:`ResNet`.
    """

    def __init__(self, stem, stages, num_classes=None, out_features=None, freeze_at=0, precision='float32'):
        """
        Args:
    ...
    def half(self):
        self.stem.half()
        self.stages = [stage.half() for stage in self.stages]
        super(DeltaResNet, self).half()
        return self
...
def build_delta_resnet_backbone(cfg, input_shape):
    ...
    device = 'cuda'
    model = DeltaResNet(stem, stages, out_features=out_features, freeze_at=freeze_at, precision=precision).to(device, memory_format=torch.channels_last)
